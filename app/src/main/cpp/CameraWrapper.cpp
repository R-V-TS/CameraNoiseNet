...
CameraWrapper::CameraWrapper(JNIEnv *env, jobject pInstance) {
    ...
    displayNoise = env->GetMethodID(cl, "displayNoiseLevel", "(F)V");
}
...
void CameraWrapper::displayNoiseLevel(float noise) {
    JNIEnv *env_n = getJniEnv();
    env_n->CallVoidMethod(mObjectRef, displayNoise, noise);
}